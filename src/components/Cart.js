...
const Cart = () => {
    ...
    const payment = () => {
        ...
        console.log("orden enviada a la DB:", order)
        ...
        .then(result => Swal.fire(
                'Order Created!',
                `Your order was generated under the ID: ${result.id}`,
                'success'
            ))
            .then(context.removeAllItems())
            .then(setModal((prevState) => !prevState))

            // NOTA: se puede revisar la orden enviada a la DB en la consola.
                    ...
                    <div className="border-b border-gray-300 md:flex-row flex-col flex justify-between items-start w-full pb-8 ">
                        ...
                        <div className="flex self-center justify-evenly xl:justify-around space-x-8 items-start w-full">
                            <p className="text-base xl:text-lg leading-6">
                                $ {item.price} 
                                    <span className="text-red-300 hidden xl:inline line-through pl-2"> 
                        ...
                        <form 
                            onSubmit={(e) => context.removeItem(e)} 
                            data-id={item.id} 
                            data-color={item.color} 
                            data-size={item.size} 
                            data-identificator={item.identificator} 
